...
uint8_t 				command;
uint8_t 				gync_x_receive[10];
uint8_t 				gync_y_receive[10];
uint8_t 				gync_z_receive[10];
Fog_Transmit_t 	Fog_Transmit;
uint8_t 				adc_xyz[4];
uint16_t				adc_value;
uint16_t				dac_value;
uint8_t 				pid_transmit_receive[DMA2_CHANNEL7_BUFFER_SIZE];
...
void wk_nvic_config(void)
{
  ...
  nvic_irq_enable(DMA2_Channel7_IRQn, 0, 0);
}
